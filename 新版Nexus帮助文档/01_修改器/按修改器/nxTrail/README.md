...
已启用（Enabled）关闭后会停止生成新的拖尾结果，当前发射器列表、连接方式和样条设置会继续保留。
...
- 厚度样条（Thickness Spline）编辑
- `样条最大值（Spline Max）`
- `样条时间（Spline Time）`

##### 样条最大值（Spline Max）

`样条最大值（Spline Max）` 控制厚度样条（Thickness Spline）可达到的最大粗细。
当前版本插件默认值是 `0.01`。
...
`样条时间（Spline Time）` 控制厚度样条（Thickness Spline）沿拖尾被采样的时间范围。
当前版本插件默认值是 `30.0`。
